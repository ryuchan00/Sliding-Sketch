...
for i in $(seq 1.0 1.0 5.0); do
  ...
  # output=$(./main ../../../../data/sx-stackoverflow.txt $mymemory $input_num $cycle $element_count) # ./mainの出力を変数に保存
  output=$(./main ../../../../data/webdocs.dat $mymemory $input_num $cycle $element_count $cycle $element_count) # ./mainの出力を変数に保存
